...
describe('Law Cards content', () => {
  ...
  test('the 6 spec-mandated mappings are correct', () => {
    ...
    expect(CASE_TO_LAW['bcc-three-and-01']).toBe('law-bcc')
    expect(CASE_TO_LAW['bcc-cost-intuition-01']).toBe('law-bcc')
    expect(CASE_TO_LAW['mcdc-tutorial-01']).toBe('law-mcdc')
    expect(CASE_TO_LAW['mcdc-altitude-disengage-01']).toBe('law-mcdc')

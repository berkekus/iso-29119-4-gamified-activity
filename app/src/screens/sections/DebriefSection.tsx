...
interface DebriefSectionProps extends SectionProps {
  onNavigateOut: (screen: Screen) => void
}

export default function DebriefSection({ onNavigateOut }: DebriefSectionProps) {
  const {
  ...
  const handleNext = () => {
    ...
    if (nextId) {
      try {
        loadCaseById(nextId)
        // phase resets to 'briefing' via loadCaseById — CasePlayScreen scrolls to top
      } catch (err) {
        console.error('[DebriefSection] Failed to load next case', nextId, err)
      }
    } else {
    ...
    <div>
      <div className="responsive-row" style={{ maxWidth: 900, margin: '0 auto', gap: 30 }}>
        ...
        <div style={{ width: 260, display: 'flex', flexDirection: 'column', gap: 16 }}>
          ...
          <div style={{ fontFamily: PIXEL_FONT, fontSize: 8, color: TC.grey, textAlign: 'center' }}>
            {techniqueLabel}
          </div>
        </div>
      </div>
